...
- CRM business data plane: the bundled CEO CRM plugin owns company-scoped customer Accounts, Contacts, Pipelines, Deals, Activities, Evidence, Work Links, and Outbox records in its plugin namespace. `companies`, agent identity/runs, approvals, credentials, and execution work remain core-owned; CRM work links reference Paperclip issue IDs rather than introducing a task runtime or duplicate issue state.
